...
export interface Expense {
  ...
  /** Set when this expense was auto-generated by a recurring payment rule
   * rather than added directly - lets the UI point back to that rule
   * instead of offering to create a duplicate one. Null for normal expenses. */
...
export type RecurringPaymentStatus = "ACTIVE" | "PAUSED" | "ENDED";

/**
 * A user-configured (or accepted-from-suggestion) recurring payment rule.
 * The backend uses this to auto-generate/pre-fill expenses on schedule -
 * it does NOT itself represent a single expense.
 */
export interface RecurringPayment {
  ...
  /** Who the expense should be split with / paid to when this fires, mirrors CreateExpensePayload shape. */
  paidBy: string;
  splitType: SplitType;
  participants: ExpenseParticipant[];
  frequency: RecurringFrequency;
  /** Day-of-month (1-31) for MONTHLY/YEARLY, day-of-week (0-6, 0=Sun) for WEEKLY/BIWEEKLY. */
  intervalAnchor: number;
  startDate: string; // ISO date
  /** Null = repeats indefinitely until paused/ended by the user. */
  endDate: string | null;
  status: RecurringPaymentStatus;
  nextOccurrenceDate: string; // ISO date, next date an expense will be generated
  lastGeneratedExpenseId: string | null;
  /** How this rule was created - lets the UI show "Set up from a suggestion" provenance. */
  source: "MANUAL" | "SUGGESTION_ACCEPTED";
  /** Whether to auto-create the expense on the due date vs. just remind the user to add it. */
  autoCreate: boolean;
  reminderEnabled: boolean;
  /** Days before nextOccurrenceDate to send a reminder, only relevant if reminderEnabled. */
...
export type UpdateRecurringPaymentPayload = Partial<
  Omit<CreateRecurringPaymentPayload, "participants">
> & {
  participants?: CreateRecurringPaymentPayload["participants"];
  status?: RecurringPaymentStatus;
};

/**
 * A backend-detected candidate for "you keep paying this every month" -
 * generated by pattern analysis over past expenses (see backend prompt doc),
 * not something the user configured. Surfaced until accepted or dismissed.
 */
export interface RecurringSuggestion {
  id: string;
  /** Representative title, e.g. the most common/most recent title in the matched cluster. */
  suggestedTitle: string;
  /** Representative amount - the median of the matched cluster, since amounts can vary slightly. */
  ...
  /** 0-1. Only surfaced to the user when >= 0.9 (see backend doc `confidenceScore` derivation). */
  confidenceScore: number;
  /** IDs of the past expenses that fed this suggestion, most recent first - shown as evidence in the UI. */
  matchedExpenseIds: string[];
  matchedExpenseCount: number;
  /** ISO date of the earliest and most recent matched expense, for the "since March" copy. */
  firstSeenDate: string;
  lastSeenDate: string;
  /** Predicted next date if the user accepts, prefilled into the create form. */
