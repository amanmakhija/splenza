...
export function ProfileScreen() {
  ...
  const menuItems = [
    { label: "Preferred currency", value: "INR" },
    { label: "Notifications", value: "" },
    { label: "Privacy policy", value: "" },
    { label: "About Splentra", value: "" }
  ];
    ...
    <SafeAreaView style={[styles.flex, { backgroundColor: theme.background }]} edges={["top"]}>
      ...
      <View style={[styles.menu, { backgroundColor: theme.surface, borderColor: theme.border }]}>
        {menuItems.map((item, idx) => (
          <Pressable
            key={item.label}
            style={[styles.menuRow, idx !== menuItems.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border }]}
          >
            <Text style={[styles.menuLabel, { color: theme.textPrimary }]}>{item.label}</Text>
            <View style={styles.menuRight}>
              {item.value ? <Text style={{ color: theme.textMuted, marginRight: 6 }}>{item.value}</Text> : null}
      ...
      <Pressable onPress={() => logout()} style={[styles.logoutButton, { borderColor: theme.danger }]}>
      ...
      <View style={styles.footer}>
        ...
        <Text style={{ color: theme.textMuted, fontSize: 12 }}>Splentra v1.0.0</Text>
      </View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 72, height: 72, borderRadius: 36, alignItems: "center", justifyContent: "center", borderWidth: 1, marginBottom: 12 },
  avatarText: { fontSize: 28, fontWeight: "800" },
  name: { fontSize: 18, fontWeight: "700" },
  email: { fontSize: 13, marginTop: 2 },
  menu: { marginHorizontal: 20, borderRadius: 16, borderWidth: 1, overflow: "hidden" },
